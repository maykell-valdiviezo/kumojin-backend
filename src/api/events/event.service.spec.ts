...
import { getDataSource, initializeDatabase } from "../../database/database";
  ...
  describe("List the Events", () => {
    describe("getEvents", () => {
      it("should return the list of events", async () => {
        ...
        const events = getDataSource().createEntityManager().create(Event, seedData);
        await getDataSource().manager.save(events);

        const results: Event[] = await getEvents();
